<template>
  <div>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div @drop="onDrop($event)" @dragenter.prevent @dragover.prevent class="grid gap-2 grid-cols-1 bg-indigo-400 rounded shadow-4xl">
      ...
      <div class="grid gap-2 grid-cols-1 bg-indigo-400 rounded shadow-4xl" @drop="onDrop($event)" @dragenter.prevent @dragover.prevent>
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="grid gap-2 grid-cols-1 bg-indigo-400 rounded shadow-4xl" @drop="onDrop($event)" @dragenter.prevent @dragover.prevent>
        ...
        <div draggable="true" @dragstart="startDrag($event, data, 'in')" class="bg-indigo-800 rounded text-white m-2 p-4 h-20" v-for="data in user.worker" :key="data.id" v-if="!exist_team.includes(data.id) && !occupied_worker.includes(data.id)">
          <img v-if="data.photo_path" class="w-10 rounded-full inline" :src="path + data.photo_path" alt="" />
          <img v-else class="w-10 rounded-full inline" :src="path + 'users/dummy-profile.png'" alt="" />
